...
/// Per-applet parameters injected into the shared PKCE flow.
#[derive(Debug, Clone, Copy)]
pub struct OAuthParams<'a> {
    /// OAuth scope URL, e.g. `https://www.googleapis.com/auth/gmail.metadata`.
    pub scope: &'a str,
    /// Full HTML body served on the loopback redirect after a successful
    /// authorization.
    pub success_html: &'a str,
}
...
pub async fn start_oauth_flow(
    ...
    let (auth_url, csrf) = client
        ...
        .add_scope(Scope::new(params.scope.to_owned()))
    ...
    let success_html = params.success_html.to_owned();
    let (code, state) = tokio::time::timeout(
        Duration::from_mins(5),
        wait_for_redirect(listener, success_html),
    )
    .await
